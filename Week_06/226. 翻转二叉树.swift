...
func inorderTraversal(_ root: TreeNode?) -> [Int] {
    ...
    var newNode = root
    var stack = [TreeNode]()
    while newNode != nil || !stack.isEmpty {
        if(newNode != nil){
            stack.append(newNode!)
            newNode = newNode?.left
        }else{
            newNode = stack.removeLast()
            resultArr.append(newNode!.val)
...
var rootnode:TreeNode? = Solution.resumeNodeTree([1,3,5,3,2,9], [5,3,3,1,2,9])
